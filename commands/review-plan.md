...
Ask the `cursed:cursed-worker` subagent to forward this review-plan request. (The agent is plugin-namespaced — use the exact id `cursed:cursed-worker` with the Agent tool.)
...
Dispatch to `cursed:cursed-worker`. Present the result:

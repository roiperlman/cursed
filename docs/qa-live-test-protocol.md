...
7. **For `review` with `needsInlineDiff: true` adapters, SCOPE contained the diff.** The MCP tool input on the `tool_use` block reveals `target` but not the rendered SCOPE. To inspect SCOPE without instrumenting cursed, pair the testbed with a direct `@modelcontextprotocol/sdk` `Client` (see `test/smoke/mcp-startup.test.mjs` for the pattern). Capture the `vars.SCOPE` passed to `runPanel`. Assert `SCOPE.includes('--- DIFF ---')`. This is the check that would have caught ROI-69.

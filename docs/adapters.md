...
`cursed` shells out to a CLI to talk to non-Claude models. Today that CLI is `cursor-agent`; tomorrow it might also be `codex` or `gemini-cli`. The **adapter** is the seam that lets us add a new CLI without touching `runOne`, `panel.mjs`, or any of the four user-facing commands.
...
**`parseStream(raw)`** — Async, takes the child's stdout (a string, or `null`/`undefined`). Returns a [`ParsedRun`](../scripts/lib/types.d.ts). Must be tolerant: empty input, malformed JSON lines, mid-stream truncation all need to produce a well-formed `ParsedRun` with `errors: [...]` populated rather than throwing. Cursor's NDJSON parser at [`scripts/lib/adapters/cursor/parse.mjs`](../scripts/lib/adapters/cursor/parse.mjs) is the reference; capture event shapes for new CLIs into `test/fixtures/streams/<adapter>/` (see fixture conventions below).

...
import { probeSetup } from '../../scripts/lib/setup.mjs';
...
describe('probeSetup', () => {
  it('reports available + authenticated on happy path (login / Keychain)', async () => {
    const exec = fakeExec({
      'cursor-agent --version': { stdout: '2026.04.17-787b533\n', exitCode: 0 },
      'cursor-agent status': { stdout: 'Logged in as alice@example.com\n', exitCode: 0 },
    });
    const result = await probeSetup({ exec, env: {} });
  ...
  it('accepts CURSOR_API_KEY env var as authentication', async () => {
    ...
    const result = await probeSetup({ exec, env: { CURSOR_API_KEY: 'sk-test' } });
    expect(result.available).toBe(true);
    expect(result.authenticated).toBe(true);
  });
